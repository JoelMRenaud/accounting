...
def trial_excel(con):
    ...
    for x in range(account_num):
        if int(accounts[x][3]) != 0:
            ws['A' + str(x - limiter +5)] = accounts[x][1]
            ws['E' + str(x-limiter+5)] = accounts[x][0]
            if int(accounts[x][3]) > 0:
                ws['F' + str(x-limiter+5)] = abs(int(accounts[x][3]))
                sumLeft += abs(int(accounts[x][3]))
            if int(accounts[x][3]) < 0:
                ws['G' + str(x-limiter+5)] = abs(int(accounts[x][3]))
                sumRight += abs(int(accounts[x][3]))
        else:
            account_num -= 1
            limiter += 1

    ws['F' + str(account_num + 5)] = sumLeft
    ws['G' + str(account_num + 5)] = sumRight
    ...
    workbook.save(filename="spreadsheets/trial.xlsx")